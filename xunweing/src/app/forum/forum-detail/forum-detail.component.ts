...
export class ForumDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.name = this.ls.get('name');
      ...
      let that=this;
      that.fs.getDetail(title,function (title) {
        that._detail=title[0];

        if(that.ls.get('token')!=''){
          that.fs.searchgz([{"atedtelephone":that._detail.telephone},{"attelephone":that.ls.get('telephone')}],function (result) {
            if(result[0].atid>0){
              $('.sp1').html('已关注');
            }
          })}
